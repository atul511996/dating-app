...
let insertPicture = async(user_id , url)=>{
    try{
        let selPicture = await db.exe_qry(`select * from images where user_id = '${user_id}'`)
        if(selPicture.length == 0)
         await db.exe_qry(`insert into images (user_id , image , status) values(${user_id} , '${url.file}' , 1)`)
        else
         db.exe_qry(`update images set image = ${url} where user_id = ${user_id}`)
        return "Success"
    }catch(err){
        return err;
    }
